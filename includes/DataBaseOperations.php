<?php
    class DataBaseOperations{
        ...
        *****************************************************************************/
        ...
        public function setAddTermSolution($termId, $expertId, $termType, $subpart, $superpart) {
            $stmt = $this->con->prepare("
                INSERT INTO addtermsolution
                VALUES( NULL, ?, ?, ?, ?, ?);
            ;");
            $stmt->bind_param("sssss", $termId, $expertId, $termType, $subpart, $superpart);
            return $stmt->execute();
        }

        public function addSynonyms($termId, $expertId, $synonyms) {
            foreach($synonyms as $synonym) {
                $stmt = $this->con->prepare("
                    INSERT INTO addtermsynonyms
                    VALUES( NULL, ?, ?, ?);
                ;");
                $stmt->bind_param("sss", $termId, $expertId, $synonym);
                if (!$stmt->execute()) {
                    return false;
                }
            }
